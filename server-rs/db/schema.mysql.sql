...
CREATE TABLE IF NOT EXISTS users (
  ...
  telegram_id VARCHAR(64) COMMENT 'Telegram Chat ID',
  telegram_enabled TINYINT DEFAULT 0 COMMENT '是否启用 Telegram 推送',
  telegram_bind_code VARCHAR(64) COMMENT 'Telegram 绑定码（用于 /start 自动绑定）',
  telegram_bind_code_expires_at BIGINT COMMENT 'Telegram 绑定码过期时间戳（Unix 秒）',
...
CREATE TABLE IF NOT EXISTS message_queue (
  ...
  channel VARCHAR(32) NOT NULL COMMENT '通知通道（email/bark/telegram/...)',
...
CREATE TABLE IF NOT EXISTS telegram_register_sessions (
  ...
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  INDEX idx_tg_register_session_expires (session_expires_at),
  INDEX idx_tg_register_session_stage (stage)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
