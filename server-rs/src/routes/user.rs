...
use crate::shared_ids::{
    format_remote_account_id_for_response_text, parse_remote_account_id_list_text,
};
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            ...
            post(post_reset_subscription_token),
        )
        ...
        .route(
            ...
            post(post_two_factor_backup_codes),
        )
...
async fn get_nodes(
    ...
    let accessible_row =
        sqlx::query("SELECT COUNT(*) as total FROM nodes WHERE status = 1 AND node_class <= ?")
            .bind(user_class)
            .fetch_optional(&state.db)
            .await
            .map_err(|err| err.to_string());
    ...
    for row in nodes {
        ...
        let (server, server_port, tls_host, config_value) =
            resolve_node_connection(&node.node_config);
      ...
      "#,
        ...
        let payload = node.to_value().as_object().cloned().unwrap_or_default();
        ...
        value.insert(
            "tags".to_string(),
            json!([format!("等级{}", node.node_class)]),
        );
...
async fn get_traffic_records(
    ...
    let start_date = query.start_date.unwrap_or_default().trim().to_string();
    ...
    let where_clause = format!("WHERE {}", filters.join(" AND "));
    let count_join = if node_name.is_empty() {
        ""
    } else {
    ...
    let count_sql =
        format!("SELECT COUNT(*) as total FROM traffic_logs tl {count_join} {where_clause}");
    ...
    if traffic_total == 0 && !has_time_range && !has_node_filter {
      ...
      "#
        ...
        let daily_total_sql =
            format!("SELECT COUNT(*) as total FROM daily_traffic dt {daily_where}");
        ...
        let pages = if limit > 0 {
            ((daily_total as f64) / (limit as f64)).ceil() as i64
        } else {
            1
        };
        return success(
    ...
    let pages = if limit > 0 {
        ((traffic_total as f64) / (limit as f64)).ceil() as i64
    } else {
        1
    };
    success(
    ...
    for row in rows {
        ...
        if let Some(date) = date {
            data_map.insert(
                date,
                (
                    row.try_get::<Option<i64>, _>("upload_traffic")
                        .unwrap_or(Some(0))
                        .unwrap_or(0),
                    row.try_get::<Option<i64>, _>("download_traffic")
                        .unwrap_or(Some(0))
                        .unwrap_or(0),
                    row.try_get::<Option<i64>, _>("total_traffic")
                        .unwrap_or(Some(0))
                        .unwrap_or(0),
                ),
            );
...
async fn post_two_factor_setup(
    ...
    if user.two_factor_enabled == 1
        && user.two_factor_secret.as_deref().unwrap_or("").is_empty() == false
    {
...
async fn get_referrals(
    ...
    let rebate_total = user_row
        ...
        .unwrap_or(0.0);
    let invite_code = user_row.as_ref().and_then(|row| {
    ...
    let rebate_eligible = user_row
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<i64>, _>("rebate_eligible")
                .ok()
                .flatten()
        })
        .unwrap_or(0)
...
async fn get_tickets(
    ...
    let items: Vec<Value> = rows
        .into_iter()
        .map(|row| build_ticket_value(&row, false))
        .collect();
    success(
        json!({
          "items": items,
...
async fn get_ticket_detail(
    ...
    success(
        json!({ "ticket": build_ticket_value(&ticket_row, true), "replies": replies }),
        "Success",
    )
    .into_response()
}
...
async fn get_audit_overview(
  ...
  .await;
    ...
    let enabled_rules = rules_stats
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<i64>, _>("enabled_rules")
                .ok()
                .flatten()
        })
        .unwrap_or(0);
    ...
    fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
        Self {
            ...
            two_factor_backup_codes: row
                ...
                .ok()
                .flatten(),
            two_factor_temp_secret: row
                .try_get::<Option<String>, _>("two_factor_temp_secret")
                .ok()
                .flatten(),
        }
    }
    ...
    fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
        Self {
            ...
            transfer_enable: row
                ...
                .unwrap_or(0),
            reg_date: row
                .try_get::<Option<NaiveDateTime>, _>("reg_date")
                .ok()
                .flatten(),
            last_login_time: row
                .try_get::<Option<NaiveDateTime>, _>("last_login_time")
                .ok()
                .flatten(),
            last_login_ip: row
            ...
            invite_code: row
                ...
                .flatten(),
            invited_by: row
                .try_get::<Option<i64>, _>("invited_by")
                .unwrap_or(Some(0))
                .unwrap_or(0),
            invite_limit: row
            ...
            register_ip: row
                .try_get::<Option<String>, _>("register_ip")
                .ok()
                .flatten(),
            two_factor_enabled: row
                .try_get::<Option<i64>, _>("two_factor_enabled")
...
async fn get_user_password_info(
    state: &AppState,
    user_id: i64,
) -> Result<Option<UserPasswordRow>, String> {
...
async fn list_referral_rows(
    ...
    let total_row =
        sqlx::query("SELECT COUNT(*) as total FROM referral_relations WHERE inviter_id = ?")
            .bind(inviter_id)
            .fetch_optional(&state.db)
            .await
            .map_err(|err| err.to_string())?;
...
fn build_ticket_value(row: &sqlx::mysql::MySqlRow, include_content: bool) -> Value {
    ...
    let status = row
        ...
        .flatten()
        .unwrap_or_else(|| "open".to_string());
    let last_reply_at = format_datetime(
        row.try_get::<Option<NaiveDateTime>, _>("last_reply_at")
            .ok()
            .flatten(),
    );
    let created_at = format_datetime(
        row.try_get::<Option<NaiveDateTime>, _>("created_at")
            .ok()
            .flatten(),
    );
    let updated_at = format_datetime(
...
impl SharedIdRow {
    fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
        let remote_account_id = row
            ...
            .flatten()
            .or_else(|| {
                row.try_get::<Option<i64>, _>("remote_account_id")
                    .ok()
                    .flatten()
                    .map(|value| value.to_string())
            })
...
async fn fetch_shared_id(client: &reqwest::Client, record: SharedIdRow) -> Value {
    ...
    if let Some(obj) = value.as_object_mut() {
        ...
        if !matched_accounts.is_empty() {
            ...
            obj.insert(
                "account".to_string(),
                matched_accounts.first().cloned().unwrap_or(Value::Null),
            );
...
async fn update_subscription_token(
    state: &AppState,
    user_id: i64,
    token: &str,
) -> Result<(), String> {
...
impl NodeRow {
    fn from_row(row: &sqlx::mysql::MySqlRow) -> Self {
        Self {
            ...
            node_class: row
                .try_get::<Option<i64>, _>("node_class")
                .unwrap_or(Some(0))
                .unwrap_or(0),
            node_bandwidth: row
...
fn resolve_node_connection(node_config: &str) -> (String, i64, String, Value) {
    ...
    let client = parsed.get("client").cloned().unwrap_or_else(|| json!({}));
    ...
    let port_value = client
        ...
        .or_else(|| {
            config.get("port").and_then(|value| {
                value
                    .as_i64()
                    .or_else(|| value.as_str().and_then(|v| v.parse::<i64>().ok()))
            })
...
async fn aggregate_system_traffic(state: &AppState, record_date: &str) -> Result<(), String> {
    ...
    let total_download = row
        .as_ref()
        .and_then(|row| {
            row.try_get::<Option<i64>, _>("total_download")
                .ok()
                .flatten()
        })
        .unwrap_or(0);
...
async fn list_login_logs(state: &AppState, user_id: i64, limit: i64) -> Result<Vec<Value>, String> {
    let rows = sqlx::query(
    r#"
...
async fn get_two_factor_user(
    state: &AppState,
    user_id: i64,
) -> Result<Option<UserTwoFactorRow>, String> {
    let row = sqlx::query(
        r#"
...
async fn set_two_factor_temp_secret(
    state: &AppState,
    user_id: i64,
    secret: &str,
) -> Result<(), String> {
